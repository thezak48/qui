...
export function getCommonTags(torrents: Torrent[]): string[] {
  ...
  const firstTorrentTags = torrents[0].tags? (Array.isArray(torrents[0].tags)? torrents[0].tags: torrents[0].tags.split(",").map((t: string) => t.trim())
  ).filter((t: string) => t): []
  ...
  return firstTorrentTags.filter((tag: string) => 
    torrents.every(torrent => {
      const torrentTags = torrent.tags? (Array.isArray(torrent.tags)? torrent.tags: torrent.tags.split(",").map((t: string) => t.trim())
      ): []
